...
class ProductComponent extends Component
{
    ...
    public function updateData()
    {
        ...
        $product->unit_id =  is_array($this->product['unit_id']) ? $this->product['unit_id']['value'] : $this->product['unit_id'];
        $product->product_img = $this->product['product_img'];
        $product->brand_id = is_array($this->product['brand_id']) ? $this->product['brand_id']['value'] : $this->product['brand_id'];
        $product->category_id =  is_array($this->product['category_id']) ? $this->product['category_id']['value'] : $this->product['category_id'];
        ...
        $product->barcode_type = is_array($this->product['barcode_type']) ? $this->product['barcode_type']['value'] : $this->product['barcode_type'];
        $product->created_by = auth()->user()->id;
        $product->is_active = $this->product['is_active'];

        $product->save();
        return redirect()->route('products')->with('success', 'Product' . ($this->product['id'] ? ' Updated' : ' Created') . ' Successfully');
    }
