<main>


    <div class="row">
        <div class="col-xl-12">
            <div class="card custom-card">
                <div class="card-header justify-content-between">
                    <div class="card-title"> Manage User Roles </div>
                    <div class="d-flex">
                        @can('add_role')


                        <a href="{{ route('user-role-permissions-create') }}"
                            class="btn btn-sm btn-primary btn-wave waves-light waves-effect waves-light"><i
                                class="ri-add-line fw-semibold align-middle me-1"></i> Create Role</a>

                        @endcan


                    </div>
                </div>
                <div class="card-body">
                    <div class="table-responsive">
                        <table class="table text-nowrap table-bordered  table-striped table-hover">
                            <thead class="table-primary">
                                <tr>
                                    <th scope="col">No</th>
                                    <th scope="col">Name</th>
                                    <th scope="col" class="w-50">Permissions</th>
                                    @can('super-admin')
                                    <th scope="col">Created Date</th>
                                    @endcan
                                    <th scope="col">Action</th>
                                </tr>
                            </thead>
                            <tbody>
                                @forelse ($roles as $role)
                                <tr>
                                    <td>{{ $role->id }}</td>

                                    <td class="fw-bold">{{ $role->name }}</td>
                                    <td>

                                        @forelse ($role->permissions as $permission)
                                            @php
                                                $badgeClass = str_contains($permission->display_name, 'Delete') ? 'danger' : (str_contains($permission->display_name, 'Edit') ? 'primary' : (str_contains($permission->display_name, 'View') ? 'dark' : 'success'));
                                            @endphp

                                            <span
                                                class="badge bg-{{ $badgeClass }} mb-1">{{ $permission->display_name }}</span>

                                        @empty
                                        @endforelse

                                    </td>
                                    @can('super-admin')
                                    <td>{{ $role->created_at->diffForHumans() }}</td>
                                    @endcan
                                    <td>
                                        <div class="hstack gap-2 fs-15">
                                            {{-- <a href="javascript:void(0);" class="btn btn-icon btn-sm btn-success"><i
                                                    class="ri-eye-line"></i></a> --}}
                                            @can('edit_role')
                                            <a href="{{ route('user-role-permissions',$role->id) }}"
                                                class="btn btn-icon btn-sm btn-info"><i class="ri-edit-line"></i></a>
                                            @endcan
                                            @can('delete_role')
                                            <span class="btn btn-icon btn-sm btn-danger"
                                                wire:click.prevent="deleteConfirm({{ $role->id }})"><i
                                                    class="ri-delete-bin-6-line"></i></span>
                                            @endcan

                                        </div>
                                    </td>
                                </tr>

                                @empty

                                @endforelse

                            </tbody>
                        </table>
                    </div>
                </div>
            </div>
        </div>
    </div>

</main>
@section('title', 'User Roles')


@push('custom-style')
@endpush

@push('custom-script')
<script src="https://unpkg.com/sweetalert/dist/sweetalert.min.js"></script>

<script>

    document.addEventListener('livewire:initialized', () => {

        window.addEventListener('swal:modal', event => {

            swal({
                title: event.detail.title,
                text: event.detail.text,
                icon: event.detail.type,
            });

        });

        window.addEventListener('swal:confirm', event => {
            swal({
                    title: event.detail[0].title,
                    text: event.detail[0].text,
                    icon: event.detail[0].type,
                    buttons: true,
                    dangerMode: true,
                })
                .then((willDelete) => {
                    if (willDelete) {
                        @this.dispatch('delete', {
                            id: event.detail[0].id
                        });
                    }
                });
        });
    });

</script>
@endpush
